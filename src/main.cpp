...
//
    for (int j = 0; j < 1; j++) {
        ...
        float lossSum = 0;
#pragma omp parallel for schedule(auto) num_threads(32) reduction(+:lossSum)
        ...
        std::cout << std::left << std::setw(20) << elapsed.count() << "ops:"
                  << 1024 * 1024 * (512 * 33 / elapsed.count()) << std::endl;
    }
