...
void assign_input(Position &p, Input &input, Data& output){
    ...
    int w_piece_offset = active == WHITE ? 0 : 10 * 64 * 64;
    int b_piece_offset = active == WHITE ? 10 * 64 * 64 : 0;
    ...
    output(0) = active == WHITE ? w_pieces - b_pieces : b_pieces - w_pieces;
}
...
//    write_positions_bin("F:\\OneDrive\\ProgrammSpeicher\\CLionProjects\\Koivisto\\resources\\other\\d8set.bin", &positions);
    ...
    for(int i = 0; i < 300; i++){
        ...
        for(int batch = 0; batch < batch_count; batch ++){
            ...
            printf("\repoch# %-10d batch# %-5d/%-10d loss=%-16.12f speed=%-7d eps", i, batch, batch_count, loss, (int)(batchsize / diff.count()));
            std::cout << std::flush;
        }
        std::cout << std::endl;
