...
int                      mappingIndex(Square psq, Piece p, Square kingSquare, Color view) {
    ...
    return relativeSquare + pieceType * pieceTypeFactor + (pieceColor == view) * pieceColorFactor + kingSide * kingSideFactor;
}
...
#pragma omp parallel for schedule(auto) num_threads(4)
    for (int i = 0; i < positions->header.position_count; i++) {
        mapPosition(positions->positions[i], &batch_stm[input_size * i], &batch_nstm[input_size * i], &wdl_values[i], &cp_values[i]);
    }
...
void addSource(const std::string& file_name) { sources.push_back(file_name); }
void begin() {
...
float* cpMemory() { return cp_values; }
float* wdlMemory() { return wdl_values; }
}