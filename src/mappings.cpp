...
int nnue::index(Color active, Color kingColor, Color pieceColor, Square ksq, Square psq, PieceType p) {
...
int nnue::index_factor_1(Color active, Color kingColor, Color pieceColor, Square ksq, Square psq, PieceType p) {
...
void nnue::assign_input(Position& p, Input& input, Data& output, bool original) {
    ...
    while (it.hasNext()) {
        ...
        if (active == WHITE) {
            if (!original) {
                input.indices[0 + index]               = nnue::index(active, WHITE, pieceColor, wksq, it.sq, it.piece % 6);
                input.indices[piece_count * 2 + index] = nnue::index(active, BLACK, pieceColor, bksq, it.sq, it.piece % 6);
                input.indices[piece_count * 1 + index] = nnue::index_factor_1(active, WHITE, pieceColor, wksq, it.sq, it.piece % 6);
                input.indices[piece_count * 3 + index] = nnue::index_factor_1(active, BLACK, pieceColor, bksq, it.sq, it.piece % 6);
            } else {
                input.indices[0 + index]               = nnue::index(active, WHITE, pieceColor, wksq, it.sq, it.piece % 6);
                input.indices[piece_count * 1 + index] = nnue::index(active, BLACK, pieceColor, bksq, it.sq, it.piece % 6);
            }
        } else {
            if (!original) {
                input.indices[piece_count * 2 + index] = nnue::index(active, WHITE, pieceColor, wksq, it.sq, it.piece % 6);
                input.indices[0 + index]               = nnue::index(active, BLACK, pieceColor, bksq, it.sq, it.piece % 6);
                input.indices[piece_count * 3 + index] = nnue::index_factor_1(active, WHITE, pieceColor, wksq, it.sq, it.piece % 6);
                input.indices[piece_count * 1 + index] = nnue::index_factor_1(active, BLACK, pieceColor, bksq, it.sq, it.piece % 6);
            } else {
                input.indices[piece_count * 1 + index] = nnue::index(active, WHITE, pieceColor, wksq, it.sq, it.piece % 6);
                input.indices[0 + index]               = nnue::index(active, BLACK, pieceColor, bksq, it.sq, it.piece % 6);
            }
        }
        index++;
...
int nnue::assign_inputs_batch(std::vector<Position>& positions, int offset, std::vector<Input>& inputs, std::vector<Data>& targets) {
...
void nnue::reduce_matrix(Data& original, Data& reduced) {
...
void ataxx::load_positions(const std::string& fil, std::vector<ataxx::Sample>& samples) {
    std::string   line;
    std::string   word;
...
void ataxx::assign_input(ataxx::Sample& p, Input& input, Data& output) {
...
int ataxx::assign_inputs_batch(std::vector<Sample>& positions, int offset, std::vector<Input>& inputs, std::vector<Data>& targets) {
...
int dense::index(Square psq, Piece p, Color activePlayer) {
...
void dense::assign_input(Position& p, Input& input, Data& output) {
    PositionIterator it {p};
...
int dense::assign_inputs_batch(std::vector<Position>& positions, int offset, std::vector<Input>& inputs, std::vector<Data>& targets) {
    int end = offset + inputs.size();
    if (end > positions.size())
        end = positions.size();
