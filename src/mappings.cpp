...
int nnue::assign_inputs_batch(std::vector<Position> &positions, int offset, std::vector<Input> &inputs,
...
int ataxx::assign_inputs_batch(std::vector<Sample> &positions, int offset, std::vector<Input> &inputs,
                               std::vector<Data> &targets) {
...
int dense::index(Square psq, Piece p) {
    if(p >= BLACK_PAWN)
        return psq + (p-6) * 64;
    else
        return psq + (p+6) * 64;
...
void dense::assign_input(Position &p, Input &input, Data &output) {
    ...
    while(it.hasNext()){
        it.next();
        input.indices.push_back(index(it.sq, it.piece));
    }
}

int dense::assign_inputs_batch(std::vector<Position> &positions, int offset, std::vector<Input> &inputs,
                               std::vector<Data> &targets) {
    int end = offset + inputs.size();
    if(end > positions.size()) end = positions.size();
